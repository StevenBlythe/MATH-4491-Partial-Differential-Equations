...
%  Here, our heat equation is $u_t = \alpha^2 u_{xx}$. To show we can just plug %in the solution and show that both right and left sides match up,
%  \begin{align}
%    u_t & = \alpha^2 u_{xx}\\
%    \frac{du}{dt} & = \alpha^2 \frac{d^2 u}{dx^2}
%    e^{-\lambda 2 \alpha^2 t} - \lambda^2 \alpha^2 (A \cos(\lambda x) + %B\sin(\lambda x))\\
%    & = \alpha^2 e^{-\lambda^2 \alpha^2 t} (-\lambda A \sin(\lambda x) + %\lambda B \cos(\lambda x))\\
%    & -\lambda^2(A \cos(\lambda x) + B\sin(\lambda x)) \alpha^2 e^{-\lambda^2 %\alpha^2 t}(-\lambda^2 A\cos(\lambda x) - \lambda^2 B \sin(\lambda x))\\
%    & = -\lambda^2(A \cos(\lambda x) + B\sin(\lambda x)) \alpha^2 e^{-%\lambda^2\alpha^2 t}\\
%    & = \lambda^2e^{-\lambda^2\alpha^2t} - \lambda^2(A\cos(\lambda x) + %B\sin(\lambda x))
%  \end{align}
%______________________________________________________________________________%
%
...
%
  ...
  \begin{align}
    ...
    u(x, 2) & = 10^{-5} \sin(10^6 \pi x)e^{-2 n^2 \pi^2} %-2? Not -4?
  \end{align}
\end{enumerate}
\end{document}
