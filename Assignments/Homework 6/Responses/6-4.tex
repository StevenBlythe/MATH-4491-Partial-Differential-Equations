...
Here, let us integrate the second line: %similar to the heat equation
%
\begin{align}
  % h(x) = \int g(X) dx
  h(x) & = \int g(x) \text dx
...
\begin{align}
  % v =
  % 1/2(f(x + t) + int^{x + t}_{-\infty} g(y)) dy +
  % 1/2(f(x - t) - int^x - t_-\infty g(Y) dy)
  v & =
...
Here, notice how our integrations are similar. The bottom and top intervals are the same respectively, we can combine them:
%
\begin{align}
  v & =
